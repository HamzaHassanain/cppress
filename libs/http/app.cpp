...
cppress::shared::thread_pool pool(
    std::thread::hardware_concurrency());  // Create a thread pool with available hardware threads

const std::vector<std::string> allowed_methods = {
    "GET",     "POST",     "PUT",   "DELETE", "HEAD", "OPTIONS", "PATCH", "TRACE",
    "CONNECT", "PROPFIND", "MKCOL", "COPY",   "MOVE", "LOCK",    "UNLOCK"};

void handler(std::shared_ptr<cppress::http::http_request> request,
             std::shared_ptr<cppress::http::http_response> response) {
...
void handle_request(cppress::http::http_request& request, cppress::http::http_response& response) {
    ...
    auto req_ptr = std::make_shared<cppress::http::http_request>(std::move(request));
    auto res_ptr = std::make_shared<cppress::http::http_response>(std::move(response));
    pool.enqueue([req_ptr, res_ptr]() { handler(req_ptr, res_ptr); });
}

void on_client_connected(std::shared_ptr<cppress::sockets::connection> conn) {
    // std::cout << "✅ Client connected from " << conn->get_remote_address().to_string() <<
    // std::endl;
}

void on_client_disconnected(std::shared_ptr<cppress::sockets::connection> conn) {
...
int main() {
    try {
        ...
        cppress::http::http_server server(8081, "0.0.0.0");

...
protected:
     ...
     * @note The function calls res->send() to send the response, then calls res->end() to end the
     * response.
     * @note if No matching route is found, a 404 response is sent by default, user may add their
     * own handlers
    ...
    /// @brief HTTP server callback for header processing, you may want to log or modify headers, or
    /// even close the connection
     ...
     * @note You can provide a callback via set_unhandled_exception_callback., if no callback set,
     * it will log the error and send a 500 response.

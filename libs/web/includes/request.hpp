 ...
 * the lower-level cppress::http::http_request class. It adds web framework-specific
...
public:
    ...
    virtual bool keep_alive() const {
        ...
        if (std::find(req_connection_values.begin(), req_connection_values.end(), keep_alive) !=
            req_connection_values.end()) {
            return true;
        }
     ...
     */
    virtual std::map<std::string, std::string> get_params() const { return request_params; }

    /**
     * @brief Clear all request parameters.
     *
     */
    virtual void clear_params() { request_params.clear(); }

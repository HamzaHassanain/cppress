...
/**
 * @brief Check if a URI points to a static resource by extension, ex, ip.com/file.png is considered
 * static
 *
 * @note
...
bool is_uri_static(const std::string& uri) {
    ...
    return std::find(static_extensions.begin(), static_extensions.end(), extension) !=
           static_extensions.end();
}
...
std::vector<std::pair<std::string, std::string>> get_query_parameters(const std::string& uri) {
    ...
    while (std::getline(query_stream, pair, '&')) {
        ...
        if (equal_pos != std::string::npos) {
            std::string name = cppress::web::trim(pair.substr(0, equal_pos));
            std::string value = cppress::web::trim(pair.substr(equal_pos + 1));
...
bool body_has_malicious_content(const std::string& body, bool XSS, bool SQL, bool CMD) {
    ...
    const std::vector<std::string> xss_patterns = {
        "<script>", "</script>",       "javascript:",  "javascript%3A",
        "onerror=", "onload=",         "onclick=",     "onmouseover=",
        "eval(",    "document.cookie", "fromCharCode", "String.fromCharCode",
        "alert(",   "prompt(",         "confirm("};

    // Check for SQL injection patterns
    const std::vector<std::string> sql_patterns = {"SELECT",
                                                   "UPDATE",
                                                   "DELETE",
                                                   "INSERT",
                                                   "DROP",
    ...
    const std::vector<std::string> cmd_patterns = {
        "`",           "&&",      "||",        ";",    "|",    "$(",  ">${",
        "/etc/passwd", "/bin/sh", "/bin/bash", "curl", "wget", "nc ", "netcat"};

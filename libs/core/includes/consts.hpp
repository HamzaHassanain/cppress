 ...
 * cppress::core::config::MAX_BODY_SIZE = 5 * 1024 * 1024;  // 5 MB
 * cppress::core::config::TIMEOUT_MILLISECONDS = 3000;  // 3 seconds
 *
 * // Use constants in response handling
 * response.set_status(cppress::core::consts::HTTP_OK, "OK");
 * response.add_header(cppress::core::consts::HEADER_CONTENT_TYPE, "application/json");
 * @endcode
 */
...
size_t MAX_HEADER_SIZE = 8192;  // 8 KB by default

/// Maximum size of HTTP request content-length
size_t MAX_CONTENT_LENGTH = 10 * 1024 * 1024;  // 10 MB by default

/// Maximum size of HTTP request chunked body size
size_t MAX_CHUNKED_BODY_SIZE = 1024 * 1024 * 1024;  // 1 GB by default

/// Maximum idle time before connection cleanup
std::chrono::seconds MAX_IDLE_TIME_SECONDS = std::chrono::seconds(60);  // 60 seconds by default

/// Server socket listen backlog size
int BACKLOG_SIZE = 10000;  // Default backlog size

/// Maximum number of file descriptors for epoll/select
int MAX_FILE_DESCRIPTORS = 1000;  // Default max fds

/// Timeout for epoll/select operations in milliseconds
int TIMEOUT_MILLISECONDS = 1000;  // Default timeout
}  // namespace config

/**
 * @namespace cppress::core::consts

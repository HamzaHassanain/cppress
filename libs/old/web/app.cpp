...
using cppress::shared::methods::DELETE_;
...
public:
    ...
    Item get(int id) {
        ...
        if (items.find(id) == items.end()) {
            throw cppress::web::exception("Item not found", "NOT_FOUND", "ItemStore::get", 404,
                                          "Not Found");
        }
        return items[id];
    }
    ...
    void update(int id, const std::string& name, const std::string& description, double price) {
        std::lock_guard<std::mutex> lock(mtx);
        if (items.find(id) == items.end()) {
            throw cppress::web::exception("Item not found", "NOT_FOUND", "ItemStore::update", 404,
                                          "Not Found");
        }
    ...
    void remove(int id) {
        std::lock_guard<std::mutex> lock(mtx);
        if (items.find(id) == items.end()) {
            throw cppress::web::exception("Item not found", "NOT_FOUND", "ItemStore::remove", 404,
                                          "Not Found");
        }
        items.erase(id);
    }
    ...
    try {
        ...
        server->listen(
            []() {
                ...
                std::cout << "Visit http://localhost:3000 in your browser for API documentation"
                          << std::endl;
            },

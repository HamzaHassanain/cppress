...
socket::socket(const Protocol& protocol = Protocol::UDP) : protocol(protocol) {
 ...
 */
socket::socket(const socket_address& addr, const Protocol& protocol) : addr(addr) {
    // Create socket: ::socket(domain, type, protocol)
    // domain: AF_INET (IPv4) or AF_INET6 (IPv6)
    // type: SOCK_STREAM (TCP) or SOCK_DGRAM (UDP)
    // protocol: Usually 0 for default protocol
...
void socket::connect(const socket_address& server_address) {
    ...
    if (::connect(fd.get(), server_address.get_sock_addr(), server_address.get_sock_addr_len()) ==
        SOCKET_ERROR_VALUE) {
...
void socket::bind(const socket_address& addr) {
    ...
    if (::bind(fd.get(), this->addr.get_sock_addr(), this->addr.get_sock_addr_len()) ==
        SOCKET_ERROR_VALUE) {
...
void socket::listen(int backlog) {
    ...
    if (protocol != Protocol::TCP) {
        throw socket_exception("Listen is only supported for TCP sockets", "ProtocolMismatch",
                               __func__);
    }
...
std::shared_ptr<connection> socket::accept(bool NON_BLOCKING) {
    ...
    if (protocol != Protocol::TCP) {
        throw socket_exception("Accept is only supported for TCP sockets", "ProtocolMismatch",
                               __func__);
    }
    ...
    if (!NON_BLOCKING) {
        ...
        client_fd = ::accept4(fd.get(), reinterpret_cast<sockaddr*>(&client_addr), &client_addr_len,
...
data_buffer socket::receive(socket_address& client_addr) {
    ...
    if (protocol != Protocol::UDP) {
        throw socket_exception("receive is only supported for UDP sockets", "ProtocolMismatch",
                               __func__);
    }
    ...
    std::size_t bytes_received =
        ::recvfrom(fd.get(), buffer, sizeof(buffer), 0, reinterpret_cast<sockaddr*>(&sender_addr),
                   &sender_addr_len);
...
void socket::send_to(const socket_address& addr, const data_buffer& data) {
    ...
    if (protocol != Protocol::UDP) {
        throw socket_exception("send_to is only supported for UDP sockets", "ProtocolMismatch",
                               __func__);
    }

    // ::sendto(sockfd, buf, len, flags, dest_addr, addrlen) - send datagram
    // Returns number of bytes sent, -1 on error
    std::size_t bytes_sent = ::sendto(fd.get(), data.data(), data.size(), 0, addr.get_sock_addr(),
                                      addr.get_sock_addr_len());
...
void socket::set_reuse_address(bool reuse) {
    ...
    if (setsockopt(fd.get(), SOL_SOCKET, SO_REUSEADDR, optval_ptr, sizeof(optval)) ==
        SOCKET_ERROR_VALUE) {
        throw socket_exception(
...
/**
 * No explicit resource management needed here, we trust the socket user to close the socket, when
 * done
 */
socket::~socket() {
    disconnect();

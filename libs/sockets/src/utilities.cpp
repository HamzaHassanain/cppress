...
bool is_free_port(port p) {
    ...
    setsockopt(tcp_socket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuse),
               sizeof(reuse));
#else
...
#if defined(SOCKET_PLATFORM_WINDOWS)
    setsockopt(udp_socket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuse),
               sizeof(reuse));
#else
 ...
 */
void convert_ip_address_to_network_order(const family& family_ip, const ip_address& address,
                                         void* addr) {
#if defined(SOCKET_PLATFORM_WINDOWS)
...
bool is_socket_connected(socket_t socket) {
    ...
    int result =
        getsockopt(socket, SOL_SOCKET, SO_ERROR, reinterpret_cast<char*>(&error), &error_len);

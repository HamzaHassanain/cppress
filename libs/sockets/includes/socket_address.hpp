...
public:
     ...
     */
    explicit socket_address(const port& port_id, const ip_address& address = ip_address("0.0.0.0"),
                            const family& family_id = family(IPV4));
    ...
    std::string to_string() const { return address.get() + ":" + std::to_string(port_id.get()); }
    ...
    friend void handle_ipv4(socket_address* addr, const ip_address& address, const port& port_id,
                            const family& family_id);
    friend void handle_ipv6(socket_address* addr, const ip_address& address, const port& port_id,
                            const family& family_id);
    ...
    friend std::ostream& operator<<(std::ostream& os, const socket_address& sa) {
        os << "IP Address: " << sa.get_ip_address() << ", Port: " << sa.get_port()
           << ", Family: " << sa.get_family();
        return os;
    }
};
 ...
 */
void handle_ipv4(socket_address* addr, const ip_address& address, const port& port_id,
                 const family& family_id);
 ...
 */
void handle_ipv6(socket_address* addr, const ip_address& address, const port& port_id,
                 const family& family_id);
}  // namespace cppress::sockets